...
Stream<T> repeatLatestValueEverySecond<T>(Stream<T> originalStream) async* {
  T? latestValue;

  // Listen to the original stream to get the latest value.
  originalStream.listen((value) {
    latestValue = value;
  });

  // Create a periodic stream that emits every second.
...
class FlutterBlueApp3 extends StatelessWidget {
  const FlutterBlueApp3({Key? key}) : super(key: key);
                ...
                StreamBuilder<List<BluetoothDevice>>(
                  ...
                  builder: (c, snapshot) => Column(
                    children: (snapshot.data ?? [])
                        .map(
                          (d) => ListTile(
                        title: Text(d.localName.toString()),
                            ...
                            if (snapshot.data == BluetoothConnectionState.disconnected) {
                              return ElevatedButton(
                                ...
                                onPressed: () async {
                                  ...
                                  try {
                                    await d.connect(timeout: Duration(seconds: 5));
                                    List<BluetoothService> services = await d.discoverServices();
                                    for (BluetoothService service in services) {
                                      for (BluetoothCharacteristic characteristic in service.characteristics) {
                                        if (characteristic.uuid.toString() == "bb554156-6a67-4d42-8853-465451492462") {
                                          List<int> connectionPassword = [0x38, 0x38, 0x38, 0x38];
                                          await characteristic.write([0x8F, ...connectionPassword], withoutResponse: true);

                                          // Read the response from the device
                                          List<int> response = await characteristic.read();
                                          if (response.length >= 5 && response[0] == 0x8F) {
                                            if (response[1] == 0x38 && response[2] == 0x38 && response[3] == 0x38 && response[4] == 0x38) {
                                              // Password correct
                                              print("Password correct, device response: ${response.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join(' ')}");

                                              // Sending additional command and reading response
                                              List<int> additionalCommand = [0x8F, 0x38, 0x38, 0x38, 0x38];
                                              await characteristic.write(additionalCommand, withoutResponse: true);
                                              List<int> additionalResponse = await characteristic.read();
                                              // Process the additional response here

                                            } else {
                                              // Password wrong
                                              print("Password wrong, device response: ${response.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join(' ')}");
                                            }
                                          }
                                        }
                                      }
                  ...
                  builder: (c, snapshot) => Column(
                    children: (snapshot.data ?? [])
                        ...
                        onTap: () async {
                          ...
                          try {
                            await r.device.connect(timeout: Duration(seconds: 35));
                            List<BluetoothService> services = await r.device.discoverServices();
                            for (BluetoothService service in services) {
                              for (BluetoothService service in services) {
                                for (BluetoothCharacteristic characteristic in service.characteristics) {
                                  if (characteristic.uuid.toString() == "0000fff6-0000-1000-8000-00805f9b34fb") {
                                    List<int> dataToSend = [8, 8, 8, 8];
                                    await characteristic.write(dataToSend, withoutResponse: true);
                                    print("Done"); // Print "Done" in the log
                                  }
                                }
                              }
                            }
...
class DeviceScreen extends StatelessWidget {
        ...
        characteristicTiles: s.characteristics
            ...
            characteristic: c,
                onReadPressed: () async {
                  try {
                    await c.read();
                    // Process the read response from the device
                    // Check for your desired responses and handle accordingly
                  } catch (e) {
                ...
                descriptor: d,
                    onReadPressed: () async {
                      try {
                        ...
                        // Process the read response from the device
                        String responseText = "Read Response: ${response.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join(' ')}";
                        final snackBar = snackBarGood(responseText);
                        snackBarKeyC.currentState?.removeCurrentSnackBar();
                        snackBarKeyC.currentState?.showSnackBar(snackBar);
                      } catch (e) {
                        final snackBar = snackBarFail(prettyException("Read Error:", e));
  ...
  Widget build(BuildContext context) {
    return ScaffoldMessenger(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(device.localName.toString()),
          actions: <Widget>[
