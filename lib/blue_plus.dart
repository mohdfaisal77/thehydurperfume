...
final snackBarKeyD = GlobalKey<ScaffoldMessengerState>();
...
class BluetoothAdapterStateObserver extends NavigatorObserver {
  ...
  void didPush(Route route, Route? previousRoute) {
    ...
    if (route.settings.name == '/TrioDiff') {
  ...
  void _startListeningForResponses(BluetoothCharacteristic characteristic) {
    // Subscribe to notifications
    characteristic.setNotifyValue(true);

    // Listen for notifications
    characteristic.value.listen((data) {
      final decodedData = utf8.decode(data); // Decode the received data
      print("Received response from device: $decodedData");
    });
  }
        ...
        body: RefreshIndicator(
          onRefresh: () async {
            ...
            if (FlutterBluePlus.isScanningNow == false) {
              if (await FlutterBluePlus.isAvailable == true) {
                          ...
                          if (snapshot.data == BluetoothConnectionState.connected) {
                            return ElevatedButton(
                              ...
                              onPressed: () => Navigator.of(context).push(MaterialPageRoute(
                                  ...
                                  settings: RouteSettings(name: '/TrioDiff'))),
                            );
                          }
        ...
        characteristicTiles: s.characteristics
            ...
            onNotificationPressed: () async {
              try {
                ...
                // onSendConnectionStatusPressed: () async {
                //   try {
                //     if (device.connectionState.value == BluetoothConnectionState.connected) {
                //       await c.write([1, 2, 3, 4]); // Replace with your desired byte sequence
                //       print("Sent connection status to device.");
                //     }
                //   } catch (e) {
                //     final snackBar = snackBarFail(prettyException("Write Error:", e));
                //     snackBarKeyC.currentState?.removeCurrentSnackBar();
