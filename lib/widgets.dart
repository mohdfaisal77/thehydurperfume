...
class ScanResultTile extends StatelessWidget {
  ...
  Widget _buildTitle(BuildContext context) {
    if (result.device.localName.isNotEmpty) {
      return Column(
        ...
        children: <Widget>[
          Text(
            result.device.localName.toString(),
  ...
  String getNiceHexArray(List<int> bytes) {
    return '[${bytes.map((i) => i.toRadixString(16).padLeft(2, '0')).join(', ')}]'.toUpperCase().toString();
  }
  ...
  Widget build(BuildContext context) {
    return ExpansionTile(
      ...
      children: <Widget>[
        _buildAdvRow(context, 'Complete Local Name', result.advertisementData.localName.toString()),
        ...
        _buildAdvRow(
            ...
            (result.advertisementData.serviceUuids.isNotEmpty)
                ...
                : 'N/A'),
        _buildAdvRow(context, 'Service Data', getNiceServiceData(result.advertisementData.serviceData.cast<String, List<int>>())),
      ],
    );
  }
}
...
class CharacteristicTile extends StatefulWidget {
  ...
  final Future<void> Function()? onSendConnectionStatusPressed;
  ...
  const CharacteristicTile(
      ...
      this.onSendConnectionStatusPressed})
      : super(key: key);
